...
export default function TestBooking2() {
  ...
  const addFood = () => setFoods(v => [...v, { name: "", price: "" }]);
  const addNote = () => setNotes(v => [...v, { text: "", price: "" }]);
      ...
      <div className="card">
        ...
        <section className="section">
          ...
          <div className="typegrid">
            <button className={bookingType === "room" ? "type selected" : "type"} onClick={() => setBookingType("room")}><span className="fakeIcon">▭</span><span><b>1 ห้อง</b><small>฿{ROOM_PRICE.toLocaleString()}/คืน</small></span>{bookingType === "room" && <i>✓</i>}</button>
            <button className={bookingType === "house" ? "type selected" : "type"} onClick={() => setBookingType("house")}><span className="fakeIcon">⌂</span><span><b>เหมาหลัง</b><small>฿{HOUSE_PRICE.toLocaleString()}/คืน</small></span>{bookingType === "house" && <i>✓</i>}</button>
          </div>
        </section>

        <section className="section">
          <Title n="4" text="ผู้เข้าพักและอาหาร" />
          <div className="counter"><div><b>ผู้ใหญ่</b><small>อย่างน้อย 1 คน</small></div><div className="step"><button onClick={() => setAdults(v => Math.max(1, v - 1))}>−</button><b>{adults}</b><button onClick={() => setAdults(v => v + 1)}>＋</button></div></div>
          <div className="counter"><div><b>เด็ก</b><small>ไม่เกิน 12 ปี</small></div><div className="step"><button onClick={() => setChildren(v => Math.max(0, v - 1))}>−</button><b>{children}</b><button onClick={() => setChildren(v => v + 1)}>＋</button></div></div>

          <div className="subhead"><b>อาหาร</b><span>รวม ฿{foodTotal.toLocaleString()}</span></div>
          ...
          <button className="add" onClick={addFood}>＋ เพิ่มอาหาร</button>

          <div className="subhead notehead"><b>ค่าใช้จ่ายเพิ่มเติม / หมายเหตุ</b><span>รวม ฿{noteTotal.toLocaleString()}</span></div>
          ...
          <button className="add" onClick={addNote}>＋ เพิ่มหมายเหตุ</button>
        </section>
